...
export default function ApplicationClasses() {
  ...
  return <div>
    ...
    <div className="card"><div className="table-wrap"><table><thead><tr><th>Class</th><th>Status</th><th /></tr></thead><tbody>
      {classes.map(item => <tr key={item.id}><td>{item.name}</td><td>{item.active ? 'Active' : 'Inactive'}</td><td><button className="btn btn-outline btn-sm" onClick={() => toggle(item)}>{item.active ? 'Deactivate' : 'Activate'}</button></td></tr>)}
